...
class Gerardas231Template(BaseTemplate):
    """Template for GERARDAS231 - Emitir DAS."""
    ...
    def validate(self, dados: dict[str, Any]) -> dict[str, Any]:
        ...
        Args:
            dados: Input data dictionary with required field:
                - parcelaParaEmitir: int (parcela to emit in format YYYYMM, e.g., 202304)
        ...
        # Validate format YYYYMM (e.g., 202304 = April 2023)
        if parcela < 190000 or parcela > 210012:
            raise ValidationError("Field 'parcelaParaEmitir' must be in YYYYMM format (e.g., 202304)")
...
# Register template
TemplateRegistry.register("RELPMEI", "GERARDAS231", Gerardas231Template)
