...
class Gerardas191Template(BaseTemplate):
    """Template for GERARDAS191 - Emitir DAS."""
    ...
    def validate(self, dados: dict[str, Any]) -> dict[str, Any]:
        ...
        Args:
            dados: Input data dictionary with required field:
                - parcelaParaEmitir: int (parcela to emit in format YYYYMM, e.g., 202308)
        ...
        # Validate format YYYYMM (e.g., 202308 = August 2023)
        if parcela < 190000 or parcela > 210012:
            raise ValidationError("Field 'parcelaParaEmitir' must be in YYYYMM format (e.g., 202308)")
...
# Register template
TemplateRegistry.register("RELPSN", "GERARDAS191", Gerardas191Template)
